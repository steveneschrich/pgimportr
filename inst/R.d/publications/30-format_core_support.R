  ...
  x |>
    dplyr::mutate(
      core_support = tidyr::unite(
        dplyr::pick(dplyr::all_of(paste0("manu_supported_cores___", pg_cores()))),
        col = "X", sep=", ",remove = TRUE, na.rm=TRUE
      ) |>
        dplyr::pull("X"),

      other_support = tidyr::unite(
        dplyr::pick(dplyr::all_of(paste0("manu_supported_cores___",pg_other_support()))),
        col = "X", sep=", ", remove = TRUE, na.rm = TRUE
      ) |>
        dplyr::pull("X"),
      support = tidyr::unite(
        dplyr::pick(dplyr::all_of(paste0("manu_supported_cores___", pg_support()))),
        col = "X", sep = ", ", remove = TRUE, na.rm = TRUE
      ) |>
        dplyr::pull("X")
    )
